...
def buildOperationalPerformace(operationalPerformace, ByOccurrence, ByWeek, checkinCheckout, allOperationalPerformaceByOccurrenceAndDate):
    ...
    with tab1:
        ...
        with container2:    
            ...
            checkinCheckout['PORCENTAGEM DE CHECKIN(%)'] = ((checkinCheckout['QUANTIDADE DE CHECKIN'] * 100) / checkinCheckout['NÚMERO DE SHOWS']).map("{:.2f}%".format)
            checkinCheckout['PORCENTAGEM DE CHECKOUT(%)'] = ((checkinCheckout['QUANTIDADE DE CHECKOUT'] * 100) / checkinCheckout['NÚMERO DE SHOWS']).map("{:.2f}%".format)
